-- Script de mise à jour de la table candidatures (version simplifiée)
USE katapult_db;

-- Sauvegarde de la table existante (si elle n'existe pas déjà)
CREATE TABLE IF NOT EXISTS candidatures_backup LIKE candidatures;
...
ALTER TABLE project_teams
    ...
    RENAME COLUMN role TO member_role,
    ADD COLUMN member_expertise TEXT AFTER member_role;
    
-- Création d'index pour améliorer les performances
CREATE INDEX idx_candidature_status ON candidatures(status);
CREATE INDEX idx_candidature_user ON candidatures(user_id);
CREATE INDEX idx_candidature_submission_date ON candidatures(submission_date); 